...
def greedy(poiList, newPoI, poiDict, k=7):
    # union = []
    unionIndex = []

    for poi in poiList:
        # union.append(poiDict[poi.poiNo][poi.offset])
    ...
    if length > k:
        ...
        for i in range(length - 6):
            # j = i+1. ..., length-k+1
            for j in range(i + 1, length - 5):
                # k = j+1,...,length-k+3
                for k in range(j + 1, length-4):
                    for a in range(k + 1, length - 3):
                        for b in range(a + 1, length - 2):
                            for c in range(b+1, length - 1):
                                for d in range(c+1, length):
                                    ...
                                    tmpList.append(unionIndex[c])
                                    tmpList.append(unionIndex[d])
                                    if sum(updateUtil(tmpList, poiDict)) > maxi:
                                        ...
                                        res.append(unionIndex[c])
                                        res.append(unionIndex[d])
...
def main():
    ...
    with open("SSTD_res/index_lambda0_bound1_k7_time", "wb") as output_file:
        cPickle.dump(saveRes, output_file)

    for i in range(200):
        finalRes, divList = search(nodeList[i], data1, poiDict, 1, 1, drange=2000)
        #print divList
        saveRes.append(divList)

    with open("SSTD_res/index_lambda1_bound1_k7_time", "wb") as output_file:
        cPickle.dump(saveRes, output_file)

    for i in range(200):
        finalRes, divList = search(nodeList[i], data1, poiDict, 0.25, 1, drange=2000)
        #print divList
        saveRes.append(divList)

    with open("SSTD_res/index_lambda025_bound1_k7_time", "wb") as output_file:
        cPickle.dump(saveRes, output_file)
