...
def greedy(poiList, newPoI, poiDict, k=3):
...
def search(queryP, data, poiDict, theta, advancedFlag, topicNum = 10, drange = 2000, k = 3):
	...
	while searchQueue:
		...
		if nextStep[1] in searchQueue:
			...
			divList.append((siteCount, sum(divRes)))
...
def main():
	...
	saveRes = []
	for i in range(200):
		finalRes, divList = search(nodeList[i], data1, poiDict, 0.75, 1, drange=2000)
		#print divList
		saveRes.append(divList)

	with open("SSTD_res/index_lambda075_bound1_k3_site", "wb") as output_file:
